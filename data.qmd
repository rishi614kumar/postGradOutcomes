...
There are three types of columns in the pseoe dataset:

- id-columns, uniquely identifying each group.
- data-columns, providing earnings information about the given group.
- status-columns, providing information about the "quality" of an observation in a given data-column.
...
When looking at the missing values in the dataset, we see that they only appear in the data-columns. The id and status columns contain no missing values.
...
The next thing that we can take into account is that the dataset gives us information about the data present in the data-columns through the status-columns. Concretely, the status columns contain one of the values shown in the following table:
...
```
When looking at the earnings data and the number of employed graduates, we see that this data for graduates one year after graduation is only missing because they don't meet the Census Bureau standards. However, when looking at graduates five years after graduation, this data is also often missing because it is simply not yet available. This trend continues for graduates ten years after graduation, where the most common reason for missing data is the fact that it is simply not available. This of course makes sense, as a lot of rows in the dataset correspond to cohorts that graduated less than 5 or 10 years ago. However, all cohorts graduated at least one year ago, and thus the reason for missing values one year after graduation cannot be that it is not yet available. Lastly, we see that the "total count" columns, representing the total number of graduates in the cohort, are only missing due to unavailability.
...
```{r}
#colnames(pseof)
#pseof_cols_select <- c("y1_grads_nme", "y5_grads_nme", "y10_grads_nme", "status_y1_grads_nme", "status_y5_grads_nme", 
#                       "status_y10_grads_nme")
#pse_join <- pse_join[,c(colnames(pseoe), pseof_cols_select)]
```
